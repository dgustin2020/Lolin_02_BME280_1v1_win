/*
cmd_p.cpp
gestione parametrin utente salvati in flash (gest sensore, volume, LCD, ecc.)
...
/* usata per leggere o programmare i vari parametri utente che si trovano in flash
la stringa:
$,u,R,0 -> xx,r / w, CR LF	
0 -> xx : è il parametro operativo fa leggere, da 0 a xx (vedi tabella "tbl_user_param_rec")
r / w   : indica se parametro da leggere o da scrivere

Con parametro == '0', 
- tipo read (r), viene inviata su seriale la lista dei parametri programmati
- tipo write (w), i parametri mmodificati con quuesto comando vengono salvati in flash ("$,p,C,0,w,")
...
Scrivi prametro
"$,p,R,xx,w,val,CR LF"	dove 'xx' è il parametro utente e 'val' è il valore da programmare
in risposta abbiamo la stringa $,p,C,xx,W, Cr LF
Leggi parametro
"$,p,R,xx,r,CR LF"	dove 'xx' è il parametro utente da leggere
...
/****/
char cmd_u(char com)//_param_gest
{
  long parametro;
  int n_char;
  ...
  if (parametro >= PREF_USER_TBL_REC_MAX){//test se parametro nella list
  ...
  */
  ...
  {  //tipo read
    ...
    *proto_to++ = P_SEPARA;
    ...
    switch (parametro)
    {
      case PREF_USER_READ_ALL_IDX://richiesta invio tuttii parametri
        pref_gest_write_all_param_to_serial((T_pref_record**)&tbl_user_param_rec, com);
        break;
      default://alri parametri
        if ( (parametro < PREF_USER_TBL_REC_MAX) && (p_tbl_rec[parametro]) )//test se esite parametro 
        {//minore e esiste
          switch ( p_tbl_rec[parametro]->value_type){
            case PARAM_BOOL:
              proto_to = proto_to + sprintf(proto_to,"%d", *(char*)p_tbl_rec[parametro]->p_key_value);
              break;
            case PARAM_STRING:
  ...
  {//tipo write
    switch (parametro){
      case PREF_USER_READ_ALL_IDX://richiesta salva parametri
        /*verifica se richiesta rimetti valori default: se trovo 'd' metto default*/
        if (*proto_from == 'd'){
          pref_gest_set_all_param_default((T_pref_record**)&tbl_user_param_rec, SAVE_DEFAULT_TO_SPACE_YES);//salva valori default
        }        
        else {
          pref_gest_save_all_param_to_space((T_pref_record**)&tbl_user_param_rec, (Preferences*)&impo_user_space); //write valori attuali
        }
        break;  //end PREF_BASE_READ_ALL
      default:
        if ( (parametro < PREF_USER_TBL_REC_MAX) && (p_tbl_rec[parametro]) )//test se esite parametro 
        {//minore e esiste //richiesta imposta singolo parametro
          long* p_long;
          long  val_l;
          char* p_char;;
          char  val_c;
          ...
          {
            case PARAM_BOOL:
              if ( (*proto_from == '0') || (*proto_from == '1') )
              {//char valido
                p_char = (char*)p_tbl_rec[parametro]->p_key_value;
                *p_char = *proto_from - '0';//metto valore
                n_char = 1;
              }
              else {
